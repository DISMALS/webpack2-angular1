class HomeMedicalHistoryListCtrl {
    constructor($rootScope,$scope, $uibModal, $state, $stateParams,$timeout,medicalService,toastr,$cookies,_) {
       ...
       function changeDesensitization(type,data){
        ...
        $scope.openDetails = (row) => {
            // console.log(row)
            $scope.$emit('addTabHomeMedicalHistorytabs', {
                title: row.name,
        ...
        $scope.searchFn = (obj) => {
            ...
            angular.forEach(obj.filterConditions,(element,i) => {
                ...
                //疾病分期id转name
                //疾病分期id转name
                if(element.filterItem=='cpa' ||element.filterItem=='diseaseStage'||element.filterItem=='sod'||element.filterItem=='sodsub'||element.filterItem=='allergens' ){
...
HomeMedicalHistoryListCtrl.$inject =  ['$rootScope','$scope', '$uibModal', '$state','$stateParams','$timeout','medicalService','toastr','$cookies','_'];
...
module.exports = (ngMold) => {
    ...
    ngMold.controller('homeMedicalHistoryListCtrl', HomeMedicalHistoryListCtrl);
}