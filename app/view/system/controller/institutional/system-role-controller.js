class SystemRoleCtrl {
    constructor($scope, $state, $stateParams,systemService,$uibModal,toastr) {
        //tootip
        $("[data-toggle='tooltip']").tooltip();

        //获取角色列表
        $scope.init = (pageNo,listSize,obj) => {
            systemService.getDoctorList(pageNo,listSize).then(rps => {
                if(rps.status == 200){
        ...
        $scope.deleteRole = (item) => {
            $uibModal.open({
                ...
                systemService.deleteDoctorRole(item.roleId).then(data => {
                    if(data.status == 200){
                        toastr.success('成功删除医生角色！',null,1500);
                        $scope.init(1,1000);
                    }else{
        ...
        $scope.addOrEditRole = (item) => {
            $uibModal.open({
                ...
                systemService.saveAddEditDoctorRole(obj).then(data => {
                    if(data.status == 200){
                        if(item){ //修改角色
        ...
        $scope.save = (role,permission) => {
            ...
            systemService.saveDoctorRolePermission(obj).then(data => {
                if(data.status == 200){
        ...
        $scope.chooiseRoles = (role) => {
            let permissionArr = [];
            systemService.getPermissionByDoctorRole(role.roleId).then(data => {
                if(data.status == 200){
        ...
        $scope.roleOptions = {
            ...
            isDisabledFn: $scope.isDisableFn //启用禁用角色
        };
    }
}
SystemRoleCtrl.$inject = ['$scope', '$state', '$stateParams','systemService','$uibModal','toastr'];
