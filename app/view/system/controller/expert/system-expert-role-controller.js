class SystemExpertRoleCtrl {
    constructor($scope, $state, $stateParams,systemService,$uibModal,expertService,toastr) {
        //tootip
        $("[data-toggle='tooltip']").tooltip();

        //获取角色列表
        $scope.init = (pageNo,listSize,obj) => {
            expertService.getExpertRoleList(pageNo,listSize).then(rps => {
                if(rps.status == 200){
        ...
        $scope.deleteRole = (item) => {
            $uibModal.open({
                ...
                expertService.deleteExpertRole(item.roleId).then(data => {
                    if(data.status == 200){
                        toastr.success('成功删除专家角色！',null,1500);
                        $scope.init(1,1000);
                    }else{
        ...
        $scope.addOrEditRole = (item) => {
            $uibModal.open({
                ...
                expertService.saveAddEditExpertRole(obj).then(data => {
                    if(data.status == 200){
                        if(item){ //修改角色
        ...
        $scope.save = (role,permission) => {
            ...
            expertService.saveExpertRolePermission(obj).then(data => {
                if(data.status == 200){
        ...
        $scope.chooiseRoles = (role) => {
            let permissionArr = [];
            expertService.getPermissionByExpertRole(role.roleId).then(data => {
                if(data.status == 200){
...
SystemExpertRoleCtrl.$inject = ['$scope', '$state', '$stateParams','systemService','$uibModal','expertService','toastr'];
