class SystemAccountViewDetailsCtrl {
    constructor($scope, $stateParams,$state) {
        ...
        //edit
        $scope.edit = () => {
            $state.go('dryad.system.operating-account.details', { id: $scope.id });
        };

        //close
        $scope.close = () => {
            $state.go('dryad.system.operating-account.list');
        };


    }
}
SystemAccountViewDetailsCtrl.$inject = ['$scope', '$stateParams','$state'];
